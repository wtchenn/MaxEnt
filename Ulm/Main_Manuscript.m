...
opts = detectImportOptions(".\Data Source\Ulm_Data_Cleaned.xlsx",'ReadVariableNames',true,'PreserveVariableNames',true);

% Specify sheet and range
opts.Sheet = "sheet1";
opts.DataRange = "A2:AD4015";
...
% Import the data
Count_Table = readtable(".\Data Source\Ulm_Data_Cleaned.xlsx", opts);
...
% Specify sheet and range
opts.Sheet = "id2name";
opts.DataRange = "A2:D35";

% Specify column names and types
opts.VariableNames = ["Location_ID", "Sensor_Description_Cleaned", "Latitude", "Longitude"];
opts.VariableTypes = ["double", "char", "double", "double"];

% Specify variable properties
opts = setvaropts(opts, ["Sensor_Description_Cleaned"], "WhitespaceRule", "preserve");
opts = setvaropts(opts, ["Sensor_Description_Cleaned"], "EmptyFieldRule", "auto");

% Import the data
Sensor_Info = readtable(".\Data Source\SensorLocations.xlsx", opts, "UseExcel", false);
...
% Import the data
EdgeList = readtable(".\Data Source\Ulm_links.txt", opts);
...
% Import the data
Node_Table = readtable(".\Data Source\Ulm_nodes.csv", opts);
...
for Distance_Threshold = 300:-50:200
...
Count_Table.State(Count_Table.Weekday==7)=2

for i = 1:7%length(neighbor_y_index)
...
%check the 2456th sample for outliers mentioned in the article
...
for i = 1:length(all_nodes)
    ...
    text(temp_position(i,1)+0.00001,temp_position(i,2)+0.00001,['Sensor ',num2str(SensorIDinCount(find(SensorIDinCount2num==all_nodes(i))))],'Color','blue','FontSize',13,'FontWeight','bold');
end
...
load('./Estimation/Result_T350.mat')

idx = find(Count_Table.Hour==12);
Count_Table = Count_Table(idx,:);
Result = Result(idx,:);

Count_Table.State(find(Count_Table.Weekday==7))=2;
[a,idx] = sort(Count_Table.State)

Result(:,max(isnan(Result))==1) = [];
Count_Table = Count_Table(idx,:)
Result = Result(idx,:);

imagesc(log(1-corr(Result')))

tick = [num2str(Count_Table.Day),'-'*ones(169,1),num2str(Count_Table.Month),'-'*ones(169,1),num2str(Count_Table.Year)];
idx = setdiff(1:169,[16,34,71,75,119,136,148,161]);
tick(idx,:) = ' '
...
idx = [119,145]
line([0, 169], [119-0.5, 119-0.5], 'Color', 'k', 'LineWidth',2);
line([119-0.5, 119-0.5], [0, 169], 'Color', 'k', 'LineWidth',2);

line([0, 169], [145-0.5, 145-0.5], 'Color', 'k', 'LineWidth',2);
line([145-0.5, 145-0.5], [0, 169], 'Color', 'k', 'LineWidth',2);
